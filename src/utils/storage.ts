...
import { SAMPLE_STUDENT_CV, SAMPLE_GENERAL_CV, SAMPLE_DEVELOPER_CV, SAMPLE_HR_CV, SAMPLE_MODERN_CV, SAMPLE_BIODATA_CV } from '../data/templates';
...
export function createNewCV(templateId: CVData['templateId'] = 'ats-student', title?: string): CVData {
  ...
  if (templateId === 'general-cv') baseSample = SAMPLE_GENERAL_CV;
  else if (templateId === 'job-biodata') baseSample = SAMPLE_BIODATA_CV;
  ...
  const defaultTitle = 
    templateId === 'general-cv' ? 'My General CV' :
    ...
    templateId === 'modern-two-column' ? 'My Executive CV' :
    'My Student Resume';
